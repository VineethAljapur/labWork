...
class spacialRepeatability:
    def __init__(self, project1="MC20_1", project2="MC20_2", frames=1):
        ...
        self.depthObj1 = DP(
            self.localMasterDirectory1, self.cloudMasterDirectory, self.logFile1
        )
        self.depthObj2 = DP(
            self.localMasterDirectory2, self.cloudMasterDirectory, self.logFile2
        )
        ...
        if nlabels1:
            ...
            for i in range(1, nlabels1 + 1):
                ...
                if (
                    ...
                    and sum(~np.isnan(diff1[np.where(labels1 == i)]))
                    > self.min_bower_size
                ):
                    ma = abs(diff1[ind].mean())
                    label1 = i
        ...
        if nlabels2:
            ...
            for i in range(1, nlabels2 + 1):
                ...
                if (
                    ...
                    and sum(~np.isnan(diff2[np.where(labels2 == i)]))
                    > self.min_bower_size
                ):
                    ma = abs(diff2[ind].mean())
                    label2 = i
        ...
        if label1 and label2 and bower1.shape == bower2.shape:
            xl.append(
                [
                    ...
                    self.project2,
                    round(
                        len(np.where(bower1 + bower2 == 3)[0])
                        * 100
                        / len(np.where(bower1 == 1)[0]),
                        2,
                    ),
                    round(
                        len(np.where(bower1 + bower2 == 3)[0])
                        * 100
                        / len(np.where(bower2 == 2)[0]),
                        2,
                    ),
                ]
            )
