...
for index, row in df.iterrows():
    ...
    if label1 and label2 and bower1.shape == bower2.shape:
        ...
        try:
            downwards = round(len(np.where(overlap == 6)[0])*100/len(np.where((bower1==3) | (bower2 == 3))[0]), 2)
        except:
            downwards = 0
            
        try:
            upwards = round(len(np.where(overlap == 2)[0])*100/len(np.where((bower1==1) | (bower2 == 1))[0]), 2)
        except:
            upwards = 0
            
        try:
            both = round(len(np.where((overlap == 2) | (overlap == 6))[0])*100/len(np.where((bower1==1) | (bower1 == 3) | (bower2 == 1) | (bower2 == 3))[0]), 2)
        except:
            both = 0
        ...
        union_pixels = len(np.where((overlap == 2) | (overlap == 6) | (overlap == 4))[0])
        shared_up_down = len(np.where((overlap == 2) | (overlap == 6))[0])
        shared_down = len(np.where(overlap==6)[0])
        shared_up = len(np.where(overlap==2)[0])
        
        xl.append([row['Trial1'], row['Trial2'], downwards, upwards, both, 
                   trial1_up_down, trial1_down, trial1_up, trial2_up_down, trial2_down, trial2_up,
                   union_pixels, shared_up_down, shared_down, shared_up])
        print ('overlap of {} with {} is {}'.format(row['Trial1'], row['Trial2'], 
                round(len(np.where((overlap == 2) | (overlap == 6))[0])*100/len(np.where((bower1==1) | (bower1 == 3) | (bower2 == 1) | (bower2 == 3))[0]), 2)))
