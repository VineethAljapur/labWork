...
class DepthProcessor:
    ...
    def __init__(self, localMasterDirectory, cloudMasterDirectory, logfile):
        ...
        self.localDepthDirectory = self.localMasterDirectory + ''
        ...
        self.dayAnalysis = 'dayAnalysis.csv'
    ...
    def bowerFinder(self, frames = 1, height_threshold = 1.0, min_bower_size = 1000):
        self.loadSmoothedArray()
        data = self.smoothDepthData

        if frames > 1: ax = 1
        else: ax = 0

        start = data[:frames].mean(axis=ax)
        end = data[data.shape[0]-frames:data.shape[0]].mean(axis=ax)
        diff = start-end

        # Gaussian Filter for Height
        img = ndimage.gaussian_filter(diff.astype(np.double), (1, 1))
        blobs = abs(img) > height_threshold
        labels, nlabels = ndimage.label(blobs)
        bower = np.zeros(shape = diff.shape, dtype = 'uint16')
        ...
        if nlabels:
            ...
            for i in range(1, nlabels+1):
                ...
                #Check if the blob has minimum bower size and find blob with maximum average absolute depth difference
                if ma < abs(diff[ind].mean()) and len(labels[ind]) > min_bower_size:
                    ma = abs(diff[ind].mean())
                    label = i
            if label != -1:
                bower[np.where(labels==label)] = blobs[np.where(labels==label)]

        np.save(self.localDepthDirectory + self.bowerLocationFile + '_new', bower)
