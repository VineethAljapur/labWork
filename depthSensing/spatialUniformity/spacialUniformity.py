...
class spacialUniformity:
    def __init__(self, project='MC20_1', day_height_threshold = 0.5, min_bower_size = 1000, frames = 1):
    ...
    def getUniformity(self):
        ...
        for blob in true_blobs: blob_diff[np.where(labels==blob)] = 1
        
        trial_diff = diff1
        trial_diff = np.nan_to_num(trial_diff)
        
        tFirst = lp.frames[0].time.replace(hour = 0, minute = 0, second = 0, microsecond = 0)
        tLast = lp.frames[-1].time.replace(hour = 23, minute = 59, second = 59, microsecond = 999999)
        ...
        for day in range(lp.numDays):
            ...
            if label:
                ...
                for blob in true_blobs: blob_diff1[np.where(labels==blob)] = 1

                diff_above_thres1 = np.zeros(shape=diff.shape)
                diff_above_thres1[np.where(blob_diff1==1)] = diff[np.where(blob_diff1==1)]

                day_volume_change = sum(sum(abs(diff_above_thres1)))
            else: day_volume_change = 0
        ...
        for day in range(lp.numDays):
            ...
            ax[day][0].set_title('Combined Bower for whole trial ({})'.format(self.file))
            ...
            if day_volume_change != 0:
                ...
                unexpectedVolume = round(np.nansum(abs(np.ma.masked_array(diff_index, ~trial_mask))) * 0.1030168618 * 0.1030168618, 2)
                actualChange = round(np.nansum(abs(np.ma.masked_array(day_diff, ~trial_mask)))  * 0.1030168618 * 0.1030168618, 2)
                ...
                display[np.where(trial_mask==True)] = diff_index[np.where(trial_mask==True)]
                
                im4=ax[day][3].imshow(np.ma.masked_array(diff_index, ~trial_mask), cmap=plt.cm.coolwarm)
                ...
                im5 = ax[day][4].imshow(np.ma.masked_array(abs(diff_index), ~trial_mask), cmap=plt.cm.gray.reversed())
                plt.colorbar(im5, ax=ax[day][4], fraction=0.035, pad=0.03)
                im5.set_clim(0, 2)
                plt.hold(True)
                
                returnArray.append([self.file, day+1, buildType, uniformityIndex, int(unexpectedVolume), int(day_volume_change* 0.1030168618 * 0.1030168618)])
            else:
                ax[day][2].hold(True)
                ax[day][2].set_title('Day'+str(day+1)+' Change')
                im3 = ax[day][2].imshow(np.ma.masked_array(diff, ~trial_mask))
                ...
                im4.set_clim(np.ma.masked_array(diff1, ~trial_mask).min(), np.ma.masked_array(diff1, ~trial_mask).max())

                ax[day][4].hold(True)
                ax[day][4].set_title('No bower on day'+str(day+1))
                im5 = ax[day][4].imshow(np.full(diff.shape, np.nan))
                im5.set_clim(np.ma.masked_array(diff1, ~trial_mask).min(), np.ma.masked_array(diff1, ~trial_mask).max())
                
                returnArray.append([self.file, day+1, buildType, 0, 0, day_volume_change])
